...
def logRecord(epoch, epochTime, lr, trainRes, evalRes, extraEvalRes):
    with open(config.logFile(), "a+") as outFile:
        record = [
            epoch,
            trainRes["acc"], evalRes["val"]["acc"], evalRes['test']['acc'],
            trainRes["loss"], evalRes["val"]["loss"], evalRes['test']['loss'],
        ]
        if config.evalTrain:
        ...
        if config.extra:
            ...
            record += [extraEvalRes["test"]["acc"], extraEvalRes["test"]["loss"]]
        record += [epochTime, lr]
...
def statsToStr(stats, res, epoch, batchNum, dataLen, startTime):
    ...
    output = formatStr.format(epoch = s_epoch, batchNum = s_batchNum, dataProcessed = s_dataProcessed,
